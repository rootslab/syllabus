    ...
    var log = console.log
        ...
        , prune = function ( oinf, osyl ) {
             var version = null
                ...
                , dtype = doString( develop )
                , filter = null
                ;
            if ( dtype === ooo.str ) {
                if ( develop !== '*' ) {
                    helpers.semver = develop;
                    pruneVersion( develop );
                }
                return update( syllabus, helpers );
            }
            if ( ( dtype === ooo.obj ) &&
                 ( doString( develop.filter ) === ooo.str ) &&
                 ( filter = filters[ develop.filter ] ) ) {
                 ;
                // scan filter
                helpers.semver = filter.semver;
             //    log( filter, filters)
                pruneVersion( filter.semver );
            }
            return update( syllabus, helpers );
        }
        ;
