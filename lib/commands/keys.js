        ...
        , scan : function ( cursor, opt, cback ) {
            ...
            switch ( doString( opt ) ) {
                case ooo.obj:
                    ...
                    match = opt.match;
                    count = opt.count;
                    if ( doString( match ) === ooo.str ) list.push( 'MATCH', match );
                    if ( ( doString( count ) === ooo.str ) ||
                         ( doString( count ) === ooo.num ) ) list.push( 'COUNT', count );
                case ooo.arr:
        ...
        , sort : function ( key, opt, cback ) {
            ...
            switch ( doString( opt ) ) {
                case ooo.obj:
                    ...
                    list = [];
                    // check properties
                    if ( doString( order ) === ooo.str ) {
                        if ( /^ALPHA|ASC|DESC$/i.test( order ) ) list.push( order );
                    }
                    if ( doString( limit ) === ooo.arr ) {
                        if ( limit.length >> 1 ) list.push( 'LIMIT', limit[ 0 ], limit[ 1 ] );
                    }
                    if ( doString( by ) === ooo.str ) list.push( 'BY', by );
                    if ( doString( get ) === ooo.arr ) {
                        len = get.length;
                        for ( ; i < len; ) {
                            el = get[ i++ ];
                            if ( doString( el ) === ooo.str ) list.push( 'GET', el );
                        }
                    }
                    if ( doString( store ) === ooo.str ) list.push( 'STORE', store );
                case ooo.arr:
