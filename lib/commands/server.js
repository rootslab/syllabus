...
exports.commands = function ( encode, error ) {
    ...
    return {
        ...
        , flushall : function ( async, cback ) {
            if ( async ) return encode( 'FLUSHALL ASYNC', null, cback );
            else return encode( 'FLUSHALL', parseIntArray, cback );
        }
         // Redis >= 4.0.0 added ASYNC option
        , flushdb : function ( async, cback ) {
            if ( async ) return encode( 'FLUSHDB ASYNC', null, cback );
            else return encode( 'FLUSHDB', null, cback );
        }
