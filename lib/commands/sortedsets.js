    ...
    return {
        ...
        , zinterstore : function ( dest, keys, weights, aggregate, cback ) {
            ...
            if ( k && w && ( ( klen = k.length ) === w.length ) ) {
                ...
                if ( toString( opt ) === ooo.str ) list.push( 'AGGREGATE', opt );
        ...
        , zrevrange : function ( key, start, stop, withscores, cback ) {
            var ws = withscores
                , list = [ start, stop ]
                ;
            if ( ! key ) return error( 'ZREVRANGE', arguments );
        ...
        , zscan : function ( key, cursor, opt, cback ) {
            ...
            switch ( toString( opt ) ) {
                case ooo.obj:
                    ...
                    match = opt.match;
                    count = opt.count;
                    if ( toString( match ) === ooo.str ) list.push( 'MATCH', match );
                    if ( toString( count ) === ooo.str ) list.push( 'COUNT', count );
                case ooo.arr:
