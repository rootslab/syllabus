module.exports = ( function () {
                ...
                hash[ h ] = ( function ( f ) {
                    return function ( ...args ) {
                        let last = args[ args.length - 1 ]
                            , pobj = null
                            , p = doString( last ) !== ooo.fn ? new Promise ( ( res, rej ) => {
                                pobj = { rs: res, rj: rej };
                            } ) : null
                            , ecmd = f( ...args )
                            ;
                        return ( p && ( ecmd.pn = pobj ) && ( 1 | eat( ecmd ) ) ) ?
                               p :
                               eat( ecmd )
                               ;
                    };
    ...
    return {
        ...
        , wrap : function ( eat, promisify ) {
            ...
            if ( ! fn ) return false;
            if ( script ) {
                if ( ! promisify ) wrap( script, fn );
                // promisify if possible
                else if ( Promise || global.Promise ) {
                    if ( doString( Promise.prototype.spread ) !== ooo.fn ) {
                        // add spread to Promise if it doesn't exists
                        Promise.prototype.spread = function ( fn ) {
                            return this.then( ( args ) => Promise.all( args ) )
                            .then( ( args ) => fn.apply( void 0, args ) );
                        };
                    }
                    wrapPromise( script, fn );
                }
                return true;
            }
            return false;
