module.exports = ( function () {
    var log = console.log
        ...
        , file_load_opt = {
            //  options for Camphora#load, set encoding to utf8
            encoding : 'utf8'
            , flag : 'r'
            // custom options
            , payload : true
                ...
                files.forEach( function ( el, i, arr ) {
                    lua_cache.load( __dirname + '/scripts/' + el, cfg, function ( err, entry ) {
                        ...
                        if ( ! script ) {
                            ...
                            list.push( encode( 'SCRIPT', 'LOAD', script, null, function ( err_reply, data ) {
                                if ( err_reply ) {
                                    // Redis returns error, delete entry from cache and return
                                    return lua_cache.delete( entry.kdigest );
                                }
                                // on script load callback
                                onLoad( entry.key, data, script, i === arr.length - 1 );
                            } ) );
                        }
                        if ( ++k === flen ) {
                            next( list );
